...
function forgottenKnowledgeDragonking.onUse(player, item, fromPosition, target, toPosition, isHotkey)
	...
	if item.itemid == 9825 then
		...
		Game.createMonster("soul of dragonking zyrtarch", config.bossPosition, true, true)
		for y = 31178, 31182 do
			...
			if playerTile and playerTile:isPlayer() then
				if playerTile:getStorageValue(Storage.ForgottenKnowledge.DragonkingTimer) < os.time() then
					...
					player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "You need to wait a while, recently someone challenge Dragonking.")
					return true
				end
			end
		end
