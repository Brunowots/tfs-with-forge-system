...
local summonPositions = {
	Position(33368, 31321, 9),
	Position(33368, 31315, 9),
	Position(33360, 31315, 9),
	Position(33360, 31321, 9),
	Position(33364, 31318, 9),
	Position(33364, 31321, 9),
	Position(33364, 31315, 9),
}

local function getOberonSummonsCount()
	local count = 0
	local specs = Game.getSpectators(setting.centerRoom, false, false, 7,7,7,7)
	for _, spec in pairs(specs) do
		if spec:getName() == "Falcon Paladin" or spec:getName() == "Falcon Knight" then
			count = count + 1
		end
	end
	return count
...
local function oberonSummonsEvent()
	local boss = Creature("Grand Master Oberon")
	if not boss then
		return
	end
	local summons = {
	...
	local summonsCount = getOberonSummonsCount()
	if summonsCount < 2 then
		randomSummon = summons[math.random(#summons)]
		randomPos = summonPositions[math.random(#summonPositions)]
		Game.createMonster(randomSummon, randomPos, false, true)
	end
	addEvent(oberonSummonsEvent, 12000)
end
...
function oberonLever.onUse(creature, item, fromPosition, target, toPosition, isHotkey)
	...
	for index, removeOberon in ipairs(clearOberonRoom) do
		...
		Game.setStorageValue(GlobalStorage.OberonHealCount, 0)
		addEvent(oberonSummonsEvent, 5000)
	local players = {}
	for i = 0, 4 do
