...
local function clearBossRoom(playerId, bossId, centerPosition, rangeX, rangeY, exitPosition)
	...
	for _, spec in pairs(spectators) do
		...
		if spec:isMonster() and spec:getId() == bossId then
			spec:remove()
		end
	end
end
...
function keys.onUse(player, item, fromPosition, target, toPosition, isHotkey)
	...
	-- Let's roll
	addEvent(clearBossRoom, 60 * tmpConfig.time * 1000, player:getId(), monster:getId(), tmpConfig.centerPosition, tmpConfig.rangeX, tmpConfig.rangeY, tmpConfig.exitPosition)
	item:remove()
