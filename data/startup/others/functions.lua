...
function loadPreyMonsters()
	for index, monsterName in pairs(Game.getBestiaryList()) do
		local mType = MonsterType(monsterName)
		if mType then
			table.insert(monsterPreysName, mType:getName())
			table.insert(monsterPreysRaceId, mType:raceId())
		end
	end

	Spdlog.info("Loaded ".. (#monsterPreysName) .." prey monsters.")
end
