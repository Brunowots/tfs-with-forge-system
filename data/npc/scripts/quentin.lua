...
keywordHandler:addKeyword({'heal'}, StdModule.say, {npcHandler = npcHandler, text = 'You are hurt, my child. I will heal your wounds.'},
	function(player) 
		if player:getStorageValue(Storage.NpcHealTalkDelay) < os.time() then
			if player:getHealth() < player:getMaxHealth() then
				Creature(player:getId()):addHealth(player:getMaxHealth(), COMBAT_HEALING)
			end
			player:setStorageValue(Storage.NpcHealTalkDelay, os.time() + (30 * 60 * 60))
		end
	end,
	function(player)
