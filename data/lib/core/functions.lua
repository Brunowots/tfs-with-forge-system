...
function clearBossRoom(playerId, bossId, centerPosition, rangeX, rangeY, exitPosition)
	if not playerId or not bossId or not centerPosition or not rangeX or not rangeY or not exitPosition then return false end
...
function getStorageTime(storage)
    local total = Game.getStorageValue(storage) - os.time()
...
function getStorageTimePlayer(storage, player)
    local total = player:getStorageValue(storage) - os.time()
    local horas = math.floor(total / 3600)
    local minutos = math.floor((total - (horas * 3600)) / 60)
    local segundos = math.floor(total % 60)
