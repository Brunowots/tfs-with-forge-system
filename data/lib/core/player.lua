...
function Player.setDisplayExp(self)
	if not self then return false end
	local result = 0
	local base = 100
	local rate = getRateFromTable(experienceStages, self:getLevel(), configManager.getNumber(configKeys.RATE_EXP))
	local staminaMinutes = self:getStamina()
	local hasboost = self:getStoreXpBoost() or 0
	self:setBaseXpGain(rate * base)
end
...
function Player.feed(self, food)
	...
	if condition then
		...
		if self:isVip() then
			healthGain = healthGain * VIP_SYSTEM_CONFIG.regem_bonus
			manaGain = manaGain * VIP_SYSTEM_CONFIG.regem_bonus
		end
...
function Player:getItemsByLocation(location, onlyMarketable)
	...
	elseif location == LOCATION_PURSE then
		local purse = self:getSlotItem(CONST_SLOT_PURSE) -- 20/03/2022
