...
function convertType(type)
	local types = {
		...
		[GameStore.OfferTypes.OFFER_TYPE_DECORATION_KIT] = GameStore.ConverType.SHOW_ITEM,
...
function parseBuyStoreOffer(playerId, msg)
	...
	local pcallOk, pcallError = pcall(function()
		...
		elseif offer.type == GameStore.OfferTypes.OFFER_TYPE_DECORATION_KIT          then GameStore.processDecorationKitPurchase(player, offer.itemtype, offer.count, offer.name)
	...
	if resultId ~= false then
		repeat
			local entry = {
				...
				coinType = result.getDataInt(resultId, "coin_type"),
			}
...
local function changeDecorationKit(kit, offerId)
	local decoItemName = ItemType(offerId):getName()
	if kit then
		kit:setAttribute(ITEM_ATTRIBUTE_DESCRIPTION, "You bought this item in the Store.\nUnwrap it in your own house to create a <" .. decoItemName .. ">.")
		kit:setCustomAttribute("unWrapId", offerId)
...
function GameStore.processDecorationKitPurchase(player, offerId, offerCount, offerName)
	if offerCount > 1 then
		if player:getFreeCapacity() < ItemType(26054):getWeight(offerCount) + ItemType(2596):getWeight() then
			return error({code = 0, message = "Please make sure you have free capacity to hold this item."})
		end
	else
		if player:getFreeCapacity() < ItemType(26054):getWeight(1) then
			return error({code = 0, message = "Please make sure you have free capacity to hold this item."})
		end
	end
			...
			local function changeParcel(parcel)
				...
				if parcel then
					...
					for i = 1, offerCount do
						...
						addEvent(function() changeDecorationKit(decoKit, offerId) end, 350)
					end
				end
			end
			addEvent(function() changeParcel(parcel) end, 250)
...
function GameStore.processExpBoostPuchase(player)
	...
	if player:getStorageValue(GameStore.Storages.expBoostCount) == -1 then
...
function GameStore.processPreyBonusReroll(player, offerCount)
	if player:getPreyCards() + offerCount >= 251 then
		return error({code = 1, message = "You cannot own more than 250 prey wildcards."})
	end
