...
function Player:onMoveItem(item, count, fromPosition, toPosition, fromCylinder, toCylinder)
	...
	local restrict_items = {2599, 2597}
	if toPosition.x ~= CONTAINER_POSITION then
		local tile = Tile(toPosition)
		if tile then
			if not tile:getItemById(1777) then
				for _, iId in pairs(restrict_items) do
					if item:getId() == iId then
						self:sendCancelMessage(RETURNVALUE_NOTPOSSIBLE)
						return false
					end
				end
	...
	-- Store Inbox
	-- Vindo do storeInbox, indo para equipament slots : não deixar
	...
	if toPosition.y > 11 then
		...
		if (containerTo) then
			if (containerTo:getId() == ITEM_STORE_INBOX) or (containerTo:getParent():isContainer() and containerTo:getParent():getId() == ITEM_STORE_INBOX and containerTo:getId() ~= ITEM_GOLD_POUCH) then
				self:sendCancelMessage("Sorry, not possible (try close the STORE INBOX)")
				return false
			end
			-- Gold Pouch
...
function Player:onMoveCreature(creature, fromPosition, toPosition)
	if creature:isPlayer() and trainingWeaponSys:isTraining(creature) and self:getGroup():hasFlag(PlayerFlag_CanPushAllCreatures) == false then
		self:sendCancelMessage(RETURNVALUE_NOTPOSSIBLE)
	return false
	end
	return true
...
function Player:onGainExperience(source, exp, rawExp)
	...
	-- vip exp bonus
	if self:isVip() then
		exp = exp + (exp * VIP_SYSTEM_CONFIG.exp_bonus)
	end
	...
	-- Custom Weekly Rate Event
	if CustomWeeklyEvents then
		if CustomWeeklyEvents["EXP"] then
			exp = exp * CustomWeeklyEvents["EXP"]
		end
	end
	...
	-- if doubleExp then
		-- exp = exp * 2
		-- self:setBaseXpGain(displayRate * 200)
	-- else
...
function Player:onLoseExperience(exp)
	if self:isVip() then -- vip lose experience
		exp = exp - (exp * VIP_SYSTEM_CONFIG.death_lose_xp)
	end
	return exp
end
...
function Player:onGainSkillTries(skill, tries)
	...
	if self:isVip() then -- vip skill bonus
		tries = tries + (tries * VIP_SYSTEM_CONFIG.skill_bonus)
	end
	
	-- Custom Weekly Rate Event
	if CustomWeeklyEvents then
		if CustomWeeklyEvents["SKILL"] then
			tries = tries * CustomWeeklyEvents["SKILL"]
		end
	end
	...
	if(skill == SKILL_MAGLEVEL) then -- Magic getLevel
		if EXTRA_EVENT_DOUBLE_SKILL then
			tries = tries * 2
		end
...
function Player:onApplyImbuement(imbuement, item, slot, protectionCharm)
	for slot = CONST_SLOT_HEAD, CONST_SLOT_AMMO do
		...
		if slotItem then
			if slotItem:getId() == item:getId() then
				-- self:closeImbuementWindow()
				self:sendImbuementResult(MESSAGEDIALOG_IMBUEMENT_ERROR, "You can't imbue a equipped item.")
				-- self:sendTextMessage(MESSAGE_FAILURE, "You can't imbue a equipped item.")
				return false
			end
		end
	end
	...
	for i=0, 2 do
		if i ~= slot then
			local imbueSlot = item:getImbuement(i)
			if imbueSlot then
				if imbueSlot:getName() == imbuement:getName() then
					self:sendImbuementResult(MESSAGEDIALOG_IMBUEMENT_ERROR, "An error ocurred, please reopen imbuement window.")
					print("O jogador ".. Player(self):getName() .." tentou bugar o imbuemento com WPE. (colocando o mesmo imbuemento em Slots Diferente)")
					return false
				end
			end
		end
	...
	if self:isPremium() then
		...
		if configManager.getBoolean(configKeys.STAMINA_PZ) then
			if zone == ZONE_PROTECTION then
				...
				if event then
					self:sendTextMessage(MESSAGE_STATUS, "[STAMINA] exit pz.")
					stopEvent(event)
