...
-- local staminaBonus = {
    -- period = 30*60*1000, -- Period in milliseconds
...
function Creature:onDrainHealth(attacker, typePrimary, damagePrimary, typeSecondary, damageSecondary, colorPrimary, colorSecondary)
	...
	if attacker then
		if attacker:isMonster() and self:isPlayer() then
			local cobra_exeption = {
				"Cobra Scout",
				"Cobra Vizier",
				"Cobra Assassin",
				"Gaffir",
			...
			for _, name in pairs(cobra_exeption) do
				if attacker:getName() == name then
					if self:getPosition() == Position(33389, 32674, 4) then -- player estiver dentro da fire wall
						if self:getPosition():getDistance(attacker:getPosition()) > 1 then
