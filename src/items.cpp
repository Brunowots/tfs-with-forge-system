...
void Items::parseItemNode(const pugi::xml_node& itemNode, uint16_t id)
{
    ...
    //} else if (tmpStrValue == "classification") { // Push from items.xml
		//	it.classification = pugi::cast<uint8_t>(valueAttribute.value());
...
uint16_t ItemType::getTierBonus(uint8_t tier, skills_t& skill, float& bonus) const
{
	if (tier == 0)
	{
		return 0;
	...
	if (isWeapon()){
		//onslaught (fatal)
		skill = SPECIALSKILL_ONSLAUGHT;
		bonus = calculateTierBonus(tier, 0.5f, 0.05f);
	}
	else if (isArmor()){
		//ruse (doge)
		skill = SPECIALSKILL_RUSE;
		bonus = calculateTierBonus(tier, 0.5f, 0.03f);
	}
	else if (isHelmet()){
		//momentum (cooldown reset)
		skill = SPECIALSKILL_MOMENTUM;
		bonus = calculateTierBonus(tier, 2.0f, 0.05f);
	}
	return 0;
} 
