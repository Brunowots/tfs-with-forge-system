...
private:
  ...
  void parsePreyAction(NetworkMessage &msg);
	...
	void sendResourceBalance(ResourceTypes_t type, uint64_t amount);
	void sendSaleItemList(const std::vector<ShopInfo>& shop, const TieredItemsCountMap& inventoryMap);
	...
	void sendMarketDetail(uint16_t itemId, uint8_t tier = 0);
