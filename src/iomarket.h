...
class IOMarket
{
	public:
		...
		static MarketOfferList getActiveOffers(MarketAction_t action, uint16_t itemId, bool version1280 = false, uint16_t tier = 0);
		...
		static void createOffer(uint32_t playerId, MarketAction_t action, uint32_t itemId, uint16_t amount, uint16_t tier, uint64_t price, bool anonymous);
		static void acceptOffer(uint32_t offerId, uint16_t amount);
		static void deleteOffer(uint32_t offerId);

		static void appendHistory(uint32_t playerId, MarketAction_t type, uint16_t itemId, uint16_t amount, uint16_t tier, uint64_t price, time_t timestamp, MarketOfferState_t state);
