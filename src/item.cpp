								 ...
								 const Item* item /*= nullptr*/, int32_t subType /*= -1*/, bool addArticle /*= true*/)
{
  ...
  uint8_t tier = item ? item->getTier() : 0;
	// winter 2021
	if (it.classification > 0 || tier > 0) {
		std::ostringstream classificationStr; 
		if (it.classification == 0){
			classificationStr << "other";
		} else {
			classificationStr << static_cast<uint16_t>(it.classification); // aqui acho que é it.classification
		}

		std::ostringstream tierString; 
		if (tier > 0) {
			skills_t skill = SKILL_FIRST;
			float bonusValue = 0;
			it.getTierBonus(tier, skill, bonusValue);

			if (skill > SKILL_FIRST) {
				//grep -r 'setprecision'
				if (skill > SKILL_MANA_LEECH_AMOUNT) {
					tierString << static_cast<uint16_t>(tier) << " (" << std::setprecision(2) << std::fixed << bonusValue << "% " << getSkillName(skill) << ")";
				}else {
					tierString << static_cast<uint16_t>(tier) << " (" << bonusValue << " " << getSkillName(skill) << ")";
				}
			}
		} else {
			tierString << "0";
		...
		//TODO: tiers classificationStr é o classification it self, se é 0 é other, arrumar isso

		if (item) {
			s << std::endl << "Classification: " << classificationStr.str() << " Tier: " << tierString.str();
		}
	}
...
bool Item::hasMarketAttributes() const
{
	...
	for (const auto& attr : attributes->getList()) {
		if (attr.type == ITEM_ATTRIBUTE_TIER) {
			...
			if (items[id].classification == 0 && attr.value.integer > 0) {
				return false;
			}
