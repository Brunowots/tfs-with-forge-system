...
MarketOfferList IOMarket::getActiveOffers(MarketAction_t action, uint16_t itemId, bool version1280, uint16_t tier)
{
...
MarketOfferList IOMarket::getOwnOffers(MarketAction_t action, uint32_t playerId)
{
	...
	std::ostringstream query;
	query << "SELECT `id`, `amount`, `price`, `tier`, `created`, `itemtype` FROM `market_offers` WHERE `player_id` = " << playerId << " AND `sale` = " << action;
...
HistoryMarketOfferList IOMarket::getOwnHistory(MarketAction_t action, uint32_t playerId)
{
	...
	std::ostringstream query;
	query << "SELECT `itemtype`, `amount`, `price`, `tier`, `expires_at`, `state` FROM `market_history` WHERE `player_id` = " << playerId << " AND `sale` = " << action;
...
void IOMarket::checkExpiredOffers()
{
	...
	std::ostringstream query;
	query << "SELECT `id`, `amount`, `price`, `tier`, `itemtype`, `player_id`, `sale` FROM `market_offers` WHERE `created` <= " << lastExpireDate;
...
MarketOfferEx IOMarket::getOfferByCounter(uint32_t timestamp, uint16_t counter)
{
	...
	std::ostringstream query;
	query << "SELECT `id`, `sale`, `itemtype`, `amount`, `created`, `price`, `tier`, `player_id`, `anonymous`, (SELECT `name` FROM `players` WHERE `id` = `player_id`) AS `player_name` FROM `market_offers` WHERE `created` = " << created << " AND (`id` & 65535) = " << counter << " LIMIT 1";
...
void IOMarket::createOffer(uint32_t playerId, MarketAction_t action, uint32_t itemId, uint16_t amount, uint16_t tier, uint64_t price, bool anonymous)
{
	std::ostringstream query;
	query << "INSERT INTO `market_offers` (`player_id`, `sale`, `itemtype`, `amount`, `tier`, `price`, `created`, `anonymous`) VALUES (" << playerId << ',' << action << ',' << itemId << ',' << amount << ',' << tier << ',' << price << ',' << time(nullptr) << ',' << anonymous << ')';
...
void IOMarket::appendHistory(uint32_t playerId, MarketAction_t type, uint16_t itemId, uint16_t amount, uint16_t tier, uint64_t price, time_t timestamp, MarketOfferState_t state)
{
	std::ostringstream query;
	query << "INSERT INTO `market_history` (`player_id`, `sale`, `itemtype`, `amount`, `tier`, `price`, `expires_at`, `inserted`, `state`) VALUES ("
		<< playerId << ',' << type << ',' << itemId << ',' << amount << ',' << tier << ',' << price << ','
...
bool IOMarket::moveOfferToHistory(uint32_t offerId, MarketOfferState_t state)
{
	...
	std::ostringstream query;
	query << "SELECT `player_id`, `sale`, `itemtype`, `amount`, `tier`, `price`, `created` FROM `market_offers` WHERE `id` = " << offerId;
...
{
	std::ostringstream query;
	query << "SELECT `sale` AS `sale`, `itemtype` AS `itemtype`, `tier` AS `tier`, COUNT(`price`) AS `num`, MIN(`price`) AS `min`, MAX(`price`) AS `max`, SUM(`price`) AS `sum` FROM `market_history` WHERE `state` = " << OFFERSTATE_ACCEPTED << " GROUP BY `itemtype`, `tier`, `sale`";
	DBResult_ptr result = Database::getInstance().storeQuery(query.str());
	if (!result) {
		return;
	}
